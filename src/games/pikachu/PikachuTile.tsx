...
export const PikachuTile: React.FC<PikachuTileProps> = ({ cell, onSelect }) => {
  const { tile, isSelected, isHinted, isRemoving } = cell;

  if (!tile) {
    return <div className="aspect-square w-full rounded-xl pointer-events-none opacity-0" />;
  }
  ...
  return (
    <motion.button
      ...
      animate={
        isRemoving
          ? { scale: [1, 1.25, 0], opacity: [1, 1, 0], rotate: [0, 15, -15] }
          : isSelected
          ? { scale: 1.08 }
          : isHinted
      ...
      transition={{ duration: isRemoving ? 0.25 : 0.15 }}
      className={`relative aspect-square w-full rounded-xl select-none flex flex-col items-center justify-center cursor-pointer transition-all duration-150 p-1 sm:p-1.5 focus:outline-none ${
        isSelected
          ? 'ring-2 sm:ring-4 ring-amber-400 shadow-[0_0_20px_rgba(251,191,36,0.6)] z-20 bg-surface/90'
          : isHinted
          ? 'ring-2 ring-sky-400 shadow-[0_0_16px_rgba(56,189,248,0.5)] z-10 bg-surface/80'
