  ...
  return (
    <div className="flex flex-col items-center gap-8 max-w-2xl mx-auto py-4">
      ...
      <div className="text-center">
        ...
        <p className="text-foreground-secondary mt-2 text-sm">Fill every row, column and 3×3 box with digits 1–9</p>
      </div>
  ...
  return (
    <div className="flex flex-col items-center gap-4 max-w-xl mx-auto">
      ...
      <div className="w-full flex items-center justify-between gap-4">
        {/* Back button */}
        <button onClick={goToMenu} className="flex items-center gap-1.5 text-sm text-foreground-secondary hover:text-foreground transition-colors">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="16"
            height="16"
      ...
      <div className="w-full flex items-center justify-between px-1">
        ...
        <div className="flex items-center gap-2 bg-white/[0.05] rounded-xl px-3 py-2 border border-white/10">
          ...
          <span className="font-mono font-bold text-foreground text-sm tabular-nums">{formatTime(elapsedSeconds)}</span>
        </div>

        {/* Hearts / errors */}
        <div className="flex items-center gap-1.5">
          {hearts.map((alive, i) => (
            <span key={i} className={`transition-all duration-300 ${alive ? 'text-rose-500' : 'text-foreground-muted/30'}`}>
      ...
      {/* Keyboard hint */}
      <p className="text-xs text-foreground-muted/50 mt-1 hidden sm:block">Arrow keys to navigate · N to toggle notes · Backspace to erase</p>
        ...
        <AnimatePresence>
          {gameState === 'gameover' && (
            ...
            >
              ...
              <p className="text-foreground-secondary text-sm mb-6">You made {MAX_ERRORS} mistakes. Better luck next time!</p>
              <div className="flex gap-3">
                <button
                  onClick={goToMenu}
                  className="rounded-xl border border-white/20 bg-white/[0.08] px-5 py-2.5 text-sm font-bold text-foreground hover:bg-white/[0.14] transition-all"
      ...
      {/* Numpad */}
      <Numpad onInput={handleNumpadInput} onErase={handleErase} onToggleNote={toggleNoteMode} noteMode={noteMode} remainingCounts={remainingCounts} />
