...
export function ChessBoard({
  ...
  const customSquareStyles = useMemo(() => {
    ...
    // 1. Last move highlights (yellow/amber tint)
    if (lastMove) {
    ...
    // 2. In check highlight (red radial glow)
    if (inCheckKingSquare) {
      styles[inCheckKingSquare] = {
        background: "radial-gradient(ellipse at center, rgba(239, 68, 68, 0.8) 0%, rgba(220, 38, 38, 0.3) 60%, transparent 80%)",
      };
    }
    ...
    // 4. Possible destination moves (subtle indicator dots/target rings)
  ...
  return (
    <div className="relative w-full max-w-[540px] aspect-square mx-auto select-none rounded-2xl p-2.5 bg-gradient-to-b from-slate-800/90 to-slate-900/90 border border-slate-700/60 shadow-2xl backdrop-blur-md">
      <Chessboard
        options={{
          position: fen,
